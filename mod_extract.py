...
class ModifiedActivityExtraction:
    ...
    def extract_all_activity_profiles(self, max_depth=100):
        ...
        def dfs_paths(current_node, path, activity_profile, timestep, traversed_arcs, 
            ...
            # For activities with only 1 contracted arc
            elif current_node == self.source and depth == 0:
                # Create activity profile following the contraction path
                contraction_arcs_from_source = []
                
                # Get all arcs in contraction path that start from source
                for item in self.contraction_path:
                    ...
                    if arc.startswith(f"{self.source}, "):
                        contraction_arcs_from_source.append(arc)
                
                # Process each arc from source
                for arc in contraction_arcs_from_source:
                    src, dest = arc.split(", ")
                    
                    # Create a new activity profile with just this arc
                    ...
                    # Continue exploration from the destination
                    new_path = [src, dest]
                    
                    # Advance exploration normally from this point
                    # Instead of adding directly to activity_profiles, call dfs_paths recursively
                    dfs_paths(
                        dest,  # Continue from destination
                        new_path,  # Updated path
                        new_activity_profile,  # Initial activity profile with the first arc
                        1,  # We've completed timestep 1
                        new_traversed,  # Record of traversed arcs
                        {},  # No c-attributes set yet
                        False,  # No out_list arc traversed yet
    ...
    def check_violating_arcs_status(self, traversed_arcs):
        ...
        for arc in violating_arc_names:
            ...
            elif arc not in traversed_arcs:
                status[arc] = {
    ...
    def print_activity_profiles(self):
        ...
        print(f"Activities with Deadlock: {deadlocks}")
